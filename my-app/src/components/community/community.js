  ...
  useEffect(() => {
    const fetchCommunities = async () => {
      try {
        ...
        const formattedGroups = response.data.map(community => ({
          id: community.id,
          name: community.name,
          description: community.description,
          memberCount: community.members ? community.members.length : 0,
          category: community.category || 'Development', // Default if not provided
          avatar: community.avatar || 'https://via.placeholder.com/50', // Default avatar
          createdBy: community.creator ? community.creator.name : 'Unknown'
        }));
        
        setCommunityGroups(formattedGroups);
        setFilteredGroups(formattedGroups);
        
        // You would similarly fetch discussions here
        // For now, we'll use the dummy discussions if you have them
  ...
  useEffect(() => {
    ...
    const filterContent = () => {
      ...
      const discussion = discussions.filter(disc =>
        (disc.title && disc.title.toLowerCase().includes(searchTerm.toLowerCase())) ||
        (disc.content && disc.content.toLowerCase().includes(searchTerm.toLowerCase())) &&
    ...
    filterContent();
  }, [searchTerm, selectedCategory, communityGroups, discussions]);

  const handleJoinGroup = async (groupId) => {
    try {
      if (joinedGroups.includes(groupId)) {
        // Handle leave functionality if you have an endpoint for it
        // For now, we'll just update the UI state
        setJoinedGroups(prev => prev.filter(id => id !== groupId));
      } else {
        // Get the current user's ID (you'll need to have this from auth)
        const userId = localStorage.getItem('userId'); // Example - adjust based on your auth setup
        ...
        await axios.post(`/api/communities/${groupId}/members`, { userId });
      ...
      alert('Failed to join/leave the group. Please try again.');
    }
  };
